...
- 增量缓存沪深300及手工跟踪股票的日线、换手率、估值、行业和结构化财务数字。
...
- 录入成交前校验沪深A股代码；非沪深300股票自动加入扩展股票池。
- 允许修改成交日期、股票、方向、数量、价格、费用、止损和备注，并重算持仓。
- 可选使用本地 Kronos 预测；没有模型时完整运行纯多因子版本。
- Kronos 结果按股票和交易日缓存；个股详情可单独计算并在 K 线图中查看预测区间。
...
- 默认股票池为当前沪深300成分股；成交中录入的有效非成分股会进入扩展股票池。
...
本项目是个人开发的非商业研究与编程实验项目，仅以源代码形式提供。作者不运营面向公众的证券投资咨询服务，不向用户提供实时投资建议，不代用户部署、运行或解释具体证券交易信号，也不因本项目的提供和使用直接或间接收取费用。代码中的评分、动作标签及仓位计算仅用于展示量化策略的程序实现，不构成任何投资建议。
